...
func InitLogger(logLevel string, outputLog string) {
	config := zap.NewProductionConfig()
	setOutputPaths(&config, outputLog)
	setLoggerLevel(logLevel, &config)
	setEnconderConfig(&config)
	buildConfiguration(&config)
}

func setOutputPaths(config *zap.Config, outputLog string) {
	config.OutputPaths = []string{outputLog}
}
